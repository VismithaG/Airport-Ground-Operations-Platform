...
class _AdminPanelPageState extends State<AdminPanelPage>
  ...
  Future<void> authorizeUser(String userUid) async {
    try {
      HttpsCallable callable = FirebaseFunctions.instance.httpsCallable(
        'grantAuthorizedClaim',
      );
      final result = await callable.call(<String, dynamic>{'uid': userUid});
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(result.data['message'] ?? 'Authorized successfully'),
        ),
      );
  ...
  final List<String> _allowedIpRanges = ['192.168.1.0/24', '10.0.0.0/16'];
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            labelText: 'IP Range (e.g., 172.16.0.0/12)',
          ),
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () => Navigator.pop(ctx, ipCtl.text.trim()),
  ...
  void _filterUsers() {
    ...
    setState(() {
      _displayedUsers = _users.where((u) {
        ...
        final matchesStatus =
            _userFilterStatus == 'All' || u.status == _userFilterStatus;
    ...
    setState(() {
      _displayedLogs = _logs.where((l) {
        ...
        final matchesSeverity =
            _logSeverityFilter == 'All' || l.severity == _logSeverityFilter;
  ...
  Future<void> _exportUsers() async {
    ...
    for (var u in _displayedUsers) {
      csv.writeln(
        '${u.name},${u.email},${u.role},${u.department},${u.status},${u.lastLogin}',
      );
    }
  ...
  Future<void> _exportLogs() async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Logs exported to clipboard')));
  }
  ...
  Future<void> _showUserDialog({UserData? editUser}) async {
    ...
    final roleCtl = TextEditingController(
      text: editUser?.role ?? 'Service Technician',
    );
    final deptCtl = TextEditingController(
      text: editUser?.department ?? 'Ground Operations',
    );
                    ...
                    builder: (ctx) => AlertDialog(
                      ...
                      content: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 6),
                          Text(
                            widget.currentUser?['designation']?.isNotEmpty ==
                                    true
              ...
              Row(
                children: [
                  ...
                  DropdownButton<String>(
                    ...
                    items: const ['All', 'Active', 'Inactive', 'Suspended']
                        .map((s) => DropdownMenuItem(value: s, child: Text(s)))
                        .toList(),
                    onChanged: (v) {
                      if (v == null) return;
                              ...
                              children: [
                                Tooltip(
                                  ...
                                  child: IconButton(
                                    ...
                                    onPressed: () => authorizeUser(user.uid),
                                  ),
                                ),
                                IconButton(
                                  icon: const Icon(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: _buildStatCard(
                        'High Priority',
                        '${_logs.where((l) => l.severity == 'High').length}',
                        Icons.warning,
                        Colors.red,
                      ),
                    ),
                    ...
                    SizedBox(
                      ...
                      child: _buildStatCard(
                        'Warnings',
                        '${_logs.where((l) => l.severity == 'Warning').length}',
                        Icons.error_outline,
                        Colors.orange,
                      ),
                    ),
                    ...
                    SizedBox(
                      ...
                      child: _buildStatCard(
                        'Information',
                        '${_logs.where((l) => l.severity == 'Info').length}',
                        Icons.info_outline,
                        Colors.blueAccent,
                      ),
                    ),
                ...
                children: [
                  SizedBox(
                    ...
                    child: TextField(
                      ...
                      onChanged: (v) => _filterLogs(),
                    ),
                  ),
                  DropdownButton<String>(
                    value: _logSeverityFilter,
                    items: const ['All', 'Info', 'Warning', 'High']
                        .map((s) => DropdownMenuItem(value: s, child: Text(s)))
                        .toList(),
                    onChanged: (v) {
                      if (v == null) return;
                      setState(() {
                        _logSeverityFilter = v;
                        _filterLogs();
                      });
                    },
                  ),
                  ...
                  children: _displayedLogs
                      .map(
                        (log) => Card(
                          ...
                          child: ListTile(
                            leading: Icon(log.icon, color: Colors.grey[700]),
              ...
              children: [
                _buildSettingsSection('Password Policy', Icons.vpn_key, [
                  _buildSwitchRow(
                    'Require Uppercase',
                    _requireUppercase,
                    (v) => setState(() => _requireUppercase = v),
                  ),
                  _buildSwitchRow(
                    'Require Lowercase',
                    _requireLowercase,
                    (v) => setState(() => _requireLowercase = v),
                  ),
                  _buildSwitchRow(
                    'Require Numbers',
                    _requireNumbers,
                    (v) => setState(() => _requireNumbers = v),
                  ),
                  _buildSwitchRow(
                    'Require Special Characters',
                    _requireSpecial,
                    (v) => setState(() => _requireSpecial = v),
                  ),
                  _buildInputRow(
                    'Minimum Length',
                    '$_minPasswordLength',
                    onChanged: (v) => setState(
                      () => _minPasswordLength =
                          int.tryParse(v) ?? _minPasswordLength,
                    ),
                  ),
                  _buildInputRow(
                    ...
                    onChanged: (v) => setState(
                      () => _passwordExpiryDays =
                          int.tryParse(v) ?? _passwordExpiryDays,
                    ),
                  ),
                  _buildButtonRow(
                    'Update Policy',
                    () => ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Password policy saved')),
                    ),
                  ),
                ]),
                const SizedBox(height: 16),
                _buildSettingsSection('Session Security', Icons.timer, [
                  ...
                  _buildInputRow(
                    ...
                    onChanged: (v) =>
                        setState(() => _maxConcurrentSessions = v),
                  ),
                  _buildSwitchRow(
                    'Require Re-authentication',
                    _requireReauth,
                    (v) => setState(() => _requireReauth = v),
                  ),
                  _buildSwitchRow(
                    'Logout on Browser Close',
                    _logoutOnClose,
                    (v) => setState(() => _logoutOnClose = v),
                  ),
                  _buildButtonRow(
                    'Update Session',
                    () => ScaffoldMessenger.of(context).showSnackBar(
                ...
                _buildSettingsSection('Access Control', Icons.shield, [
                  _buildInputRow(
                    ...
                    onChanged: (v) => setState(
                      () => _maxFailedAttempts =
                          int.tryParse(v) ?? _maxFailedAttempts,
                    ),
                  ),
                  _buildInputRow(
                    'Lockout Duration',
                    _lockoutDuration,
                    onChanged: (v) => setState(() => _lockoutDuration = v),
                  ),
                  _buildSwitchRow(
                  ...
                  ..._allowedIpRanges.map(
                    (ip) => ListTile(
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () =>
                            setState(() => _allowedIpRanges.remove(ip)),
                      ),
                    ),
                  ),
                  OutlinedButton.icon(
                  ...
                  children: [
                    _buildSettingsSection('Password Policy', Icons.vpn_key, [
                      ...
                      _buildButtonRow(
                        ...
                        () => ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Password policy saved'),
                          ),
                        ),
                      ),
                    ]),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ListTile(
                            ...
                            subtitle: const Text(
                              'Multiple failed logins from IP 198.51.100.42',
                          ...
                          Center(
                            child: TextButton(
                              onPressed: () {},
                              child: const Text('View All Security Events'),
                            ),
                          ),
                        ],
                      ),
              ...
              children: [
                _buildSettingsSection('General Settings', Icons.settings, [
                  ...
                  _buildInputRow(
                    'Default Timezone',
                    _defaultTimezone,
                    onChanged: (v) => setState(() => _defaultTimezone = v),
                  ),
                  _buildSwitchRow(
                ...
                _buildSettingsSection(
                  ...
                  [
                    ...
                    _buildButtonRow(
                      ...
                      () => ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Work order settings saved'),
                        ),
                      ),
                    ),
                  ],
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow('Uptime', '3 days 4 hours'),
                      _buildInfoRow('DB Status', 'Connected'),
                      _buildInfoRow('Storage', '120GB / 256GB'),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow('Next Backup', 'in 22 hours'),
                      const SizedBox(height: 12),
                      Row(
                        children: [
                          ElevatedButton(
  ...
  Widget _buildSeverityBadge(String severity) {
    ...
    return Container(
      ...
      child: Text(severity, style: TextStyle(color: c, fontSize: 12)),
    );
  }
  ...
  Widget _buildButtonRow(String label, VoidCallback onPressed) => Padding(
    padding: const EdgeInsets.only(top: 8),
    child: Row(
      children: [ElevatedButton(onPressed: onPressed, child: Text(label))],
    ),
  );
