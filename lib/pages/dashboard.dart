...
class UserInfo {
  ...
  UserInfo({
    ...
    required this.email,
    this.designation = '',
    this.department = 'Ground Operations',
  });
}
...
class DashboardPage extends StatefulWidget {
  ...
  const DashboardPage({super.key, required this.onLogout, this.currentUser});
...
class _DashboardPageState extends State<DashboardPage> {
        ...
        if (snapshot.hasData) {
          ...
          for (var doc in docs) {
            ...
            if (createdAt.isAfter(startOfWeek) ||
                createdAt.isAtSameMomentAs(startOfWeek)) {
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      if (widget.currentUser != null &&
                          (widget.currentUser!.role == 'Supervisor' ||
                        ...
                        ElevatedButton.icon(
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => ApprovalListPage(
                          ...
                          style: ElevatedButton.styleFrom(
                            backgroundColor: const Color(
                              0xFF7F1D1D,
                            ), // Slightly darker red for distinction
                            foregroundColor: Colors.white,
                            padding: const EdgeInsets.symmetric(
                              horizontal: 20,
                              vertical: 12,
                      ...
                      ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final wo = WorkOrder(
                            ...
                            details:
                                data['details']?.toString() ?? 'No Details',
  ...
  Widget _buildStatusBadge(String status) {
    ...
    switch (status) {
      case 'In progress':
        bgColor = const Color(0xFFE6EE9C);
        break;
      case 'Open':
